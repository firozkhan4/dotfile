...
def auto_filename_prompt(manager):
    suggested_filename = manager.filename_suggestion()
    default_dir = "~/Downloads"

    save_path = os.path.join(os.path.expanduser(default_dir), suggested_filename)

    manager.set_filename(save_path)


config.bind('<Ctrl-d>', 'download')  # example bind to trigger downloads

# Hook into download prompts
config.set('downloads.location.prompt', True)
config.set('downloads.location.directory', '~/Downloads')

# Attach the hook
config.pyqtSignal.connect(auto_filename_prompt)
